...
**Files:**
- Modify: `C:\Projects\IMS\monitoring\snmpsim\apex_mock.snmprec`

- [ ] **Step 1: Replace SNMP simulator with enhanced dynamic values**

Replace the entire content of `apex_mock.snmprec` with:
...
```bash
docker run --rm --network ims_ims-net alpine:latest sh -c "apk add net-snmp-tools > /dev/null 2>&1 && snmpwalk -v2c -c apex_mock ims-snmpsim:161 1.3.6.1.2.1.25.3.3.1.2"
```

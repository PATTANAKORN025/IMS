...
`pg_dump`s the live `ims` database, restores it into a throwaway `ims_dr_test` database (never touches live data), compares row counts on `devices`/`ldi_data`/`ldi_alarm_log` between live and restored, then drops the throwaway database. Pass criterion: exact row-count match.

...
**Known gap, found 2026-08-13 (first real run — see `docs/evidence/DR_DRILL_3_FINDINGS.md`):** this drill does not currently complete cleanly. `postgres/init/034-ldi-statistical-mock.sql` auto-seeds a fixed mock dataset into any fresh volume before `db-migrate` runs, which puts 7 of 37 migrations into a schema state they don't expect (they fail), which in turn blocks `node-red`/`proxy`/`alarm-api` from starting at all (dependency-gated on migration success), and the drill's own restore step then fails almost completely restoring into an already-migrated database (TimescaleDB's internal per-chunk table IDs in the dump don't match the freshly-created hypertable's own IDs). Running this drill currently requires manual recovery afterward — it is not yet a clean, unattended pass. Root-cause fix (making the 7 migrations tolerant of the init-seed baseline, or restoring before migrating) is tracked but not yet done.

...
| **Node-RED "Started flows" แต่ไม่มีข้อมูล** | ติดต่อเป้าหมาย SNMP ไม่ได้ หรือ community string ผิด | `docker exec ims-node-red node -e "const s=require('net-snmp').createSession('ims-snmpsim','apex_mock',{port:161,version:2});s.get(['1.3.6.1.2.1.1.3.0'],(e,v)=>{console.log(e||v);s.close()})"` | ยืนยัน snmpsim ทำงาน: `docker logs ims-snmpsim --tail=5`. ตรวจสอบ community string ว่าตรงกับโปรไฟล์ (`ubuntu` หรือ `windows`) |

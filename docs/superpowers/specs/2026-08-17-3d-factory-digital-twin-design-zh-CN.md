...
**目前的真实数据：** `public.devices.location` 是一个单一的平面文本列，具有 5 个真实值（`Site A - Zone 1`、`Site A - Zone 2`、`Site A - Zone 3`、`Site B - Zone 1`、`Site B - Zone 2`）——本会话中已确认。模式(schema)的任何地方都没有 `building`、`floor` 或 `zone` 列。“Factory 2” / “Factory 3” 和后缀（`DF INNER`/`DF OUTER`/`SM`）都被编码在一个字符串中，没有被分解。

...
**Real data today:** `public.devices.location` is a single flat text column with 5 real values (`Factory 2 - DF INNER`, `Factory 2 - DF OUTER`, `Factory 2 - SM`, `Factory 3 - DF INNER`, `Factory 3 - SM`) — confirmed this session. There is no `building`, `floor`, or `zone` column anywhere in the schema. "Factory 2" / "Factory 3" and the suffix (`DF INNER`/`DF OUTER`/`SM`) are both encoded inside one string, not decomposed.

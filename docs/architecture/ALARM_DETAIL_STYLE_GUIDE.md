...
- **Source text** — the real vendor `AlarmMsg`/`AlarmType` from `data/real/Machine error code list20250723.txt` or the supplemental `ldi_alarm_ms_code_clean.sql` export.
...
- **`impact`** — what this means operationally _right now_: is production blocked, is a result suspect, is this just a delay? This is the dimension `alarm_detail` alone never stated explicitly.

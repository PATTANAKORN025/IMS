...
| `GET /api/floor-raw-cad` | The drawing's own line-work, grouped into fixed layer **roles** | A reference, not evidence about the floor. Served in the **CAD's** frame — millimetres, +y up, no reflection — so the client applies the canonical transform itself and a frame error shows up in the overlay instead of being baked in. Each role carries exactly `id`, `segment_count` and `segments`; the drawing's real layer names never leave the server. Reports what it excludes. Fetched only when the overlay is switched on. |

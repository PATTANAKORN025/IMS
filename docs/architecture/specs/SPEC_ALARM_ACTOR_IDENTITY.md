...
caller has *a* valid Grafana session -- it does not currently propagate
*whose* session that is down to alarm-api.
...
if (verifiedUser && claimedActor !== verifiedUser) {
  ...
  console.warn(`actor mismatch: verified=${verifiedUser} claimed=${claimedActor}`);
}

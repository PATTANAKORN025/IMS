...
- Duplicate-timestamp sweep for `ldi_data`/`net_metrics` in this specific pass relied on the earlier audit's dedup-key results, not a fresh re-run with timestamp-only grouping -- a genuinely repeated identical timestamp for the _same_ device that differs in other columns wouldn't have been caught by either check. Flagged as a real gap, not closed here.

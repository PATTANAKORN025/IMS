...
|---|---|---|
| **Machine Snapshot** | 🟢 สมบูรณ์ | ทุก panel มีข้อมูล · หน่วยถูกทุกตัว · PE/JE PASS · Cpk แยก PE/JE · Event timeline ระดับมิลลิวินาที |
| **Manufacturing** | 🟢 ดีมาก | KPI ครบ · ตารางมีข้อมูล · compliance เขียวเป็นหลัก · RCA summary ท้ายหน้า |
| **Engineering & SPC** | 🟢 ดี | PE/JE 6+4 เส้นแยกได้ · histogram PE vs JE ซ้อนกัน · Cpk PE 1.253 / JE 2.710 |
| **Data Readiness** | 🟢 ดีเยี่ยม | ตรวจเจอปัญหาจริง 3 อย่างด้วยตัวเอง |
| **Andon** | 🟡 ครึ่งเดียว | KPI + 10 machine tiles ทำงานดี · แต่ 2 panels ว่าง |
...
for (const panel of allPanels) {
  ...
  if (rows.length === 0)
    fail(`${dashboard}/${panel.title}: query คืน 0 แถว`);

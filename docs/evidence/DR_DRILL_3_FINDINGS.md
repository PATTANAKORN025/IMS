...
2. **`scripts/dr-test.sh`'s `drill_full_recreate` rewritten.** Old flow: `docker compose up -d` (schema) then `psql < full_dump.sql` (restore) into an already-migrated database -- confirmed broken twice (restore-after-migrate: 435 errors; restore-before-migrate: 78 errors), both from the same root cause TimescaleDB's own `pg_dump` output warns about every run: a plain logical dump of a database with continuous aggregates is not reliably restorable once the target already has its own hypertables. New flow: start only `timescaledb`, run `db-migrate` alone (schema from `database/migrations/`, proven clean on empty), then restore **only** the raw `ldi_data`/`ldi_alarm_log` row data -- extracted from the dump's per-chunk `COPY` blocks and loaded through the parent table name via the new `scripts/dr-restore-table-data.py`, which sidesteps the internal chunk-ID mismatch entirely -- then start everything else.

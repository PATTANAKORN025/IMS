...
- After editing `monitoring/snmpsim/apex_mock.snmprec`
...
const session = snmp.createSession('ims-snmpsim', 'apex_mock', opts);
...
docker exec ims-node-red node -e "
const snmp = require('net-snmp');
const session = snmp.createSession('ims-snmpsim', 'apex_mock', { port: 161, timeout: 3000 });
...
docker exec ims-node-red node -e "
const snmp = require('net-snmp');
const session = snmp.createSession('ims-snmpsim', 'apex_mock', { port: 161, timeout: 5000 });
...
| `ETIMEOUT` | Wrong port or community string. Simulator listens on UDP 161, community `apex_mock` |
| `No such name` | OID not in `.snmprec` file. Check `monitoring/snmpsim/apex_mock.snmprec` |
...
- SNMP community string: `apex_mock`

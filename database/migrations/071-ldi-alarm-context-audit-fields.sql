-- ══════════════════════════════════════════════════════════════
-- 071: v_ldi_alarm_context — expose alarm_id, related_log_id, alarm_detail
-- ══════════════════════════════════════════════════════════════
-- Machine Snapshot incident-investigation upgrade (2026-08-11): every alarm
-- table showed the alarm CODE (a type identifier shared by many events) but
-- nothing that identifies THIS specific occurrence for audit/RCA purposes.
-- Adds the unique per-event log identifier (a.logid, aliased alarm_id since
-- the view already uses errorcode/alarm_msg-style unaliased names) and the
-- linked telemetry event (a.related_log_id) so an operator can reference
-- "this exact alarm" and cross-check it against the exact telemetry
-- snapshot (the same value shown as "Log ID" on the Board Traceability
-- panel) that caused it. Also exposes alarm_detail, previously joined but
-- never selected -- the master catalog's functional-description field,
-- distinct from alarm_msg, was invisible to every dashboard until now.
--
-- All three appended at the end (CREATE OR REPLACE VIEW cannot reorder or
-- insert columns mid-list, only append -- same constraint as migration 069).

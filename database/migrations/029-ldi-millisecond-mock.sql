-- ══════════════════════════════════════════════════════════════
-- Fix 1: Re-inject mock data with sub-minute jitter (ms timestamps)
-- Adds RANDOM() millisecond offset to prevent minute-aligned timestamps.
-- ══════════════════════════════════════════════════════════════
...
-- 3. HEALTHY MACHINES with sub-minute timestamp jitter
...
SELECT
    NOW() - (x * INTERVAL '1 minute') - (random() * INTERVAL '500 milliseconds'),
    '3', 'SM', m.name,
...
(VALUES (1, 'LDI-A03'), (3, 'LDI-A02'), (4, 'LDI-A01'),
         (5, 'LDI-B01'), (7, 'LDI-A05'), (9, 'LDI-B03')
) AS m(idx, name);

-- 4. THERMAL DRIFT MACHINES with sub-minute jitter
INSERT INTO public.ldi_data (
    "time", factory, process, eqp_id, mo, fpn, layer_name,
    resist_dosage, scale_x, scale_y, temperature, humidity,
    scan_speed, air_vacuum, thickness, board_no, total_board, total_time,
...
-- Correlated thermal drift alarms with jittered timestamps
...
-- 5. HUMIDITY SPIKE MACHINES with sub-minute jitter
INSERT INTO public.ldi_data (
    "time", factory, process, eqp_id, mo, fpn, layer_name,
    resist_dosage, scale_x, scale_y, temperature, humidity,
    scan_speed, air_vacuum, thickness, board_no, total_board, total_time,
...
-- Correlated humidity spike alarms with jittered timestamps
INSERT INTO public.ldi_alarm_log (logid, logdate, errorcode, errortime, equipmentid, factory, process)
SELECT 'ALG-' || LPAD(((m.idx * 10000 + x)::TEXT), 7, '0'),
    NOW() - (x * INTERVAL '1 minute') - (random() * INTERVAL '500 milliseconds'),
    '0303',
...
-- 6. VERIFY sub-minute jitter is present
SELECT eqp_id, COUNT(*) AS rows,
       MIN("time") AS earliest,
       MAX("time") AS latest,
       EXTRACT(SECOND FROM MIN("time")) AS min_sec,
       EXTRACT(MILLISECOND FROM MIN("time")) AS min_ms

...
function parseAll(items, type, state) {
    ...
    for (const item of items) {
        ...
        if (type === 'cpu' && (oid.startsWith('1.3.6.1.2.1.25.3.3.1.2.') || oid.match(/1\.3\.6\.1\.4\.1\.2636\.3\.1\.13\.1\.8\.(7|9)\.\d+\.0\.0$/))) { if (Number.isFinite(numVal) && numVal > 0) { cpu.total += numVal; cpu.count++; } continue; }
        if (type === 'temp' && (oid.startsWith('1.3.6.1.4.1.2021.13.16.2.1.7.') || oid.match(/1\.3\.6\.1\.4\.1\.2636\.3\.1\.13\.1\.7\.(7|9)\.\d+\.0\.0$/))) { if (numVal > 0 && numVal > maxTemp) maxTemp = numVal; continue; }

...
function parseAll(items, type, state) {
    ...
    for (const item of items) {
        ...
        if (type === 'cpu' && oid.startsWith('1.3.6.1.2.1.25.3.3.1.2.')) { if (Number.isFinite(numVal)) { cpu.total += numVal; cpu.count++; } continue; }
        if (type === 'temp' && oid.startsWith('1.3.6.1.4.1.2021.13.16.2.1.7.')) { if (numVal > maxTemp) maxTemp = numVal; continue; }

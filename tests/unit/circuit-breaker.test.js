 ...
 * machine names like "LDI-A01") built flow-context keys directly
...
test('checkDevice never throws for a device name with a space', () => {
    ...
    assert.strictEqual(checkDevice('LDI-A01', flowCtx), true);
});

test('recordFailure/recordSuccess never throw for a device name with a space', () => {
    const flowCtx = makeFlowCtx();
    recordFailure('LDI-A02', flowCtx);
    recordFailure('LDI-A02', flowCtx);
    assert.strictEqual(getState('LDI-A02', flowCtx), 'OPEN', 'should trip after 2 failures (FAILURE_THRESHOLD)');
    recordSuccess('LDI-A02', flowCtx);
    assert.strictEqual(getState('LDI-A02', flowCtx), 'CLOSED');
});

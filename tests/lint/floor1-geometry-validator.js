 ...
 */
const EQUIPMENT_DISPLAY_SHAPE = new Set(['MEASURED_RECTANGLE', 'UNRESOLVED']);
const EQUIPMENT_DISPLAY_SOURCE = new Set(['measured_extent']);
/**
...
for (const item of geometry.equipment || []) {
  ...
  if (!EQUIPMENT_FOOTPRINT_STATUS.has(item.footprint_status)) {
    ...
    // THE DISPLAY REPRESENTATION. The rectangle an operator sees is generated
    // from the three fields above -- position, rotation_deg and footprint --
    // and from nothing else, so this regenerates it exactly as the renderer
    // would and measures it back. This is the check that stops a "make the map
    // prettier" change from becoming a change to the floor.
    if (!EQUIPMENT_DISPLAY_SHAPE.has(item.display_shape)) {
      ...
      if (item.display_polygon !== undefined || item.display_vertices !== 4) {
        error(`${label}: a display rectangle owns no geometry -- expected no `
          + 'display_polygon and display_vertices 4, so the renderer has one source '
          + 'for where the machine is');
      }
      if (typeof item.rotation_deg === 'number') {
        ...
        if (dx > DISPLAY_TOL_M || dz > DISPLAY_TOL_M) {
          error(`${label}: the display rectangle moved the machine by `
            + `${(Math.max(dx, dz) * 1000).toFixed(3)} mm -- display may simplify a shape, `
            + 'never relocate it');
        }
        if (dw > DISPLAY_TOL_M || dd > DISPLAY_TOL_M) {
          error(`${label}: the display rectangle resized the machine by `
            + `${(Math.max(dw, dd) * 1000).toFixed(3)} mm -- the measured extent is `
            + 'authoritative');
        }
      }
      // The cost of the abstraction, and its sign. An oriented box CONTAINS
      // the outline it was measured from, so a negative error would mean the
      // rectangle clips the machine -- which the measurement cannot support.
      if (typeof item.display_area_error !== 'number' || item.display_area_error < 0) {
        error(`${label}: display_area_error must be a number >= 0, got `
          + `${item.display_area_error} -- a rectangle can only claim floor beyond the `
          + 'measured outline, never cut inside it');
      }
      if (!EQUIPMENT_DISPLAY_SOURCE.has(item.display_source)) {
        error(`${label}: display_source "${item.display_source}" is not one of `
          + `${[...EQUIPMENT_DISPLAY_SOURCE].join(', ')} -- a display rectangle must say `

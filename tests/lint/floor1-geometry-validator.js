...
/**
 * The display representation: two classes, and no geometry.
 *
 * The operator map draws every machine with a measured extent as one oriented
 * rectangle generated from that record's own centre, width, depth and
 * rotation. There is no display polygon in the document to check, which is the
 * point -- so what this validates is that no record grew one, that a marker
 * never claims a rectangle, and that the rectangle a renderer would generate
 * from the record measures back to the record.
 */
const EQUIPMENT_DISPLAY_SHAPE = new Set(['OPERATIONAL_RECTANGLE', 'UNRESOLVED']);
const EQUIPMENT_DISPLAY_SOURCE = new Set(['filtered_physical_footprint']);
/** Beyond this the record claims a body axis it is not allowed to act on. */
...
for (const item of geometry.equipment || []) {
  ...
  if (!EQUIPMENT_FOOTPRINT_STATUS.has(item.footprint_status)) {
    ...
    // THE OPERATIONAL FOOTPRINT. The rectangle an operator sees is generated
    // from position, rotation_deg, the operational size and the stated axis
    // offset, and from nothing else, so this regenerates it exactly as the
    // renderer would and measures it back. This is the check that stops a
    // "make the map cleaner" change from becoming a change to the floor.
    if (!EQUIPMENT_DISPLAY_SHAPE.has(item.display_shape)) {
      ...
      error(`${label}: has a measured footprint but its display class is UNRESOLVED`);
    } else {
      const op = item.operational_footprint;
      if (item.display_polygon !== undefined || item.display_vertices !== 4) {
        error(`${label}: an operational rectangle owns no geometry -- expected no `
          + 'display_polygon and display_vertices 4, so the renderer has one source '
          + 'for where the machine is');
      }
      if (!op || !Number.isFinite(op.width) || !Number.isFinite(op.depth)
        || op.width <= 0 || op.depth <= 0) {
        error(`${label}: MEASURED_CAD but no operational_footprint to draw it at`);
      } else if (typeof item.rotation_deg === 'number') {
        const offset = Number.isFinite(item.operational_axis_offset_deg)
          ? item.operational_axis_offset_deg : 0;
        if (Math.abs(offset) > OPERATIONAL_AXIS_LIMIT_DEG + 1e-9) {
          error(`${label}: operational_axis_offset_deg ${offset} exceeds the `
            + `${OPERATIONAL_AXIS_LIMIT_DEG} degree limit -- beyond it the CAD rotation `
            + 'must be preserved and the record flagged, not turned');
        }
        // Generated by the canonical helper, on the canonical frame: the
        // reflected frame puts the machine's own axis at MINUS the served
        // rotation, and building the box on +rotation is the bug this guards.
        // The rectangle's own measured centre: the machine's position plus the
        // delta the record publishes. A delta only exists where the body axis
        // differs from the INSERT axis -- one hull measured on two axes has two
        // extent centres -- and it is bounded by the machine's own size, so it
        // can re-measure a rectangle and never relocate a machine.
        const ox = Number.isFinite(op.offset_x) ? op.offset_x : null;
        const oz = Number.isFinite(op.offset_z) ? op.offset_z : null;
        if (ox === null || oz === null) {
          error(`${label}: operational_footprint carries no measured centre `
            + '(offset_x / offset_z) -- the renderer would draw the operational size '
            + 'at the physical centre and cut measured geometry away');
        }
        const limit = Math.hypot(op.width, op.depth) / 2;
        if ((ox !== null && Math.abs(ox) > limit) || (oz !== null && Math.abs(oz) > limit)) {
          error(`${label}: the operational centre delta (${ox}, ${oz}) is larger than the `
            + 'machine it belongs to -- that is a move, not a measurement');
        }
        // Two things move an extent centre, and both are on the record: the
        // axis it is measured on, and whether an enclosure was left out of the
        // geometry being measured. A delta with neither has no measurement
        // behind it.
        if ((ox || oz) && Math.abs(offset) < 1e-9 && !item.operational_excludes_enclosure) {
          error(`${label}: an operational centre delta with no body-axis offset and no `
            + 'excluded enclosure -- nothing was re-measured, so its centre cannot have moved');
        }
        const cx = item.position.x + (ox || 0);
        const cz = item.position.z + (oz || 0);
        const corners = blocks.twinBoxCorners(
          cx, cz, op.width, op.depth,
          item.rotation_deg + offset,
        );
        const flat = [];
        for (const [x, z] of corners) flat.push(x, z);
        const ext = blocks.orientedExtent(flat, -(item.rotation_deg + offset));
        const dx = Math.abs(ext.cx - cx);
        const dz = Math.abs(ext.cy - cz);
        const dw = Math.abs(ext.width - op.width);
        const dd = Math.abs(ext.depth - op.depth);
        if (dx > DISPLAY_TOL_M || dz > DISPLAY_TOL_M) {
          error(`${label}: the operational rectangle is not centred where it says it is, `
            + `by ${(Math.max(dx, dz) * 1000).toFixed(3)} mm`);
        }
        if (dw > DISPLAY_TOL_M || dd > DISPLAY_TOL_M) {
          error(`${label}: the operational rectangle is not the size it declares, by `
            + `${(Math.max(dw, dd) * 1000).toFixed(3)} mm`);
        }
        // An operational rectangle may be SMALLER than the physical extent --
        // that is the point of measuring on the body axis -- but it may never
        // be larger. Growing one would be claiming floor from nothing.
        if (item.footprint
          && op.width * op.depth > item.footprint.width * item.footprint.depth + 1e-6) {
          error(`${label}: the operational rectangle (${op.width} x ${op.depth}) is larger `
            + `than the measured extent (${item.footprint.width} x ${item.footprint.depth})`);
        }
      }
      // The cost of the abstraction, and its sign. An oriented extent CONTAINS
      // the hull it was measured from, so a negative error would mean the
      // rectangle cuts inside the machine.
      if (typeof item.display_area_error !== 'number' || item.display_area_error < 0) {
        error(`${label}: display_area_error must be a number >= 0, got `
          + `${item.display_area_error}`);
      }
      if (!EQUIPMENT_DISPLAY_SOURCE.has(item.display_source)) {
        error(`${label}: display_source "${item.display_source}" is not one of `
          + `${[...EQUIPMENT_DISPLAY_SOURCE].join(', ')} -- an operational rectangle must `
    ...
    if (item.display_polygon || item.display_vertices || item.display_source
      || item.operational_footprint) {
      error(`${label}: footprint is UNRESOLVED but display geometry is recorded `
        + '-- a marker carries no dimensional claim of any kind');
    }
  }

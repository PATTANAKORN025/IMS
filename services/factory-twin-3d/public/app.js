...
function buildFloor(geometry) {
  ...
  for (const item of equipment) {
    ...
    //
    // ONE SYMBOL: an oriented rectangle at the measured centre, the measured
    // width and depth, and the measured rotation. The measured outline is
    // reconciliation geometry -- it is in the record, the inspection layer
    // draws it, and it is not what a machine is drawn as on the operator map.
    //
    // The rectangle is built from item.position, item.footprint and
    // item.rotation_deg, and from NOTHING ELSE. There is no display polygon on
    // the wire and no display centre or angle anywhere in the model, which is
    // what makes "did the display layer move a machine" unanswerable in the
    // wrong direction: it has nothing to move one with.
    //
    // The box is shared and cached per size, so every machine of a given
    // extent costs one geometry between them.
    ...
    // Rotation about +Y, with the sign the canonical frame demands, and the
    // ONLY rotation convention in the client. See CAD_ROTATION_SIGN below; its
    // Node counterpart is twinBoxCorners in scripts/lib/cad-blocks.js, and the
    // regression measures the drawn mesh against corners generated by that.
    ...
    // on its origin, so it needs the half-height offset. x and z are the
    // record's own -- never adjusted, for overlap, for a room, or for anything.
...
function showEquipmentInspector(item) {
  ...
  render(
    ...
    [
      ...
      ['Rectangle claims beyond the outline', item.display_area_error == null
        ? 'not applicable'
        : `${(item.display_area_error * 100).toFixed(1)} % of the measured area`],
    ...
    + ' The shape on screen is a DISPLAY RECTANGLE: the operator map draws every machine '
    + 'whose extent was measured as one oriented rectangle, generated from the measured '
    + 'centre, width, depth and rotation above and from nothing else. It is a drawing '
    + 'convention, NOT a claim that the CAD footprint is rectangular — switch on Measured '
    + 'outlines to see the outline it was measured from. The block height is a '

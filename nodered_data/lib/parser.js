...
function calcNetRate(deviceId, currentIfaces, flowCtx) {
    const now = Date.now();
    const prevKey = 'net_prev_' + deviceId;
    const tsKey = 'net_ts_' + deviceId;
    const prevIfaces = (flowCtx ? flowCtx.get(prevKey) : {}) || {};
    const prevTs = (flowCtx ? flowCtx.get(tsKey) : (now - 10000)) || (now - 10000);
    ...
    if (flowCtx) flowCtx.set(prevKey, JSON.parse(JSON.stringify(currentIfaces)));
    if (flowCtx) flowCtx.set(tsKey, now);
    return { summary };
}

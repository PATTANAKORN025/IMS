...
drill_container_loss() {
    ...
    if [[ "$recovered" == "yes" ]]; then
        ...
        echo "Live-verified 2026-08-10: 'docker kill' does not reliably trigger Docker's"
        echo "restart policy on this environment (confirmed via 'docker events' showing"
        echo "only kill+die, no start). Falling back to a manual 'docker start' now so"
        echo "this drill doesn't leave the stack down -- that fallback is NOT the thing"
        echo "under test and does not change the FAIL verdict above."

...
- Every migration is a new, sequentially-numbered file in `database/migrations/` (currently 013–068, applied in order by the `db-migrate` service). **Never edit or renumber a migration after it's merged** — a correction is always the *next* number. See `docs/architecture/IMS_MANUFACTURING_PLATFORM_V2.md` §7 for the full versioning policy.
